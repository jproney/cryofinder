...
def main(args):
    ...
    print(f'Input stack standard deviation: {std}')
    ...
    if std1 > 0:
        print(f'Will add s1 (structural noise) with stdev {std1} targeting SNR {snr1}')
    else:
        print(f'Will not add s1 (structural noise); --std1 set to 0')
    ...
    if std2 > 0:
        print(f'Will add s2 (shot noise) with stdev {std2} targeting SNR {snr2}')
    else:
        print(f'Will not add s2 (shot noise); --std2 set to 0')

    # calculate overall final snr
    if (std1 <= 0) and (std2 <= 0):
        pass
    elif (std1 <= 0) and (std2 > 0):
        print(f'Final SNR: {snr2}')
    elif (std1 > 0) and (std2 <= 0):
        print(f'Final SNR: {snr1}')
    else:
        # cascading of noise processes according to Frank and Al-Ali (1975) & Baxter (2009) eq 2
        print(f'Final SNR: {(snr1 * snr2 / (1 + snr1 + snr2))}')

    # load CTF from pkl or prepare ctf_params array from args
    if args.ctf is not None:
        print(f'Loading CTF parameters from {args.ctf}')
        ...
        print('CTF pkl file not specified; using CTF parameters specified at command line')
    ...
    if args.df_std is not None:
        print(f'Jiggling defocus values by stdev {args.df_std}')
        df_std = np.random.normal(0, args.df_std, Nimg).astype(np.float32)
        if args.no_astigmatism:
            ...
            ctf_params[:,2] += df_std
            ctf_params[:,3] += np.random.normal(0, args.df_std, Nimg).astype(np.float32)
    ...
    if args.tilt_series:
        ...
        print('Using tilt scheme to calculate further attenuation of SNR due to increasing tilt')
        tilt_weights = np.cos(tilt_series * np.pi / 180).reshape(1, ntilts, 1, 1)

        if args.dose is not None:
            print(f'Using dose {args.dose}e-/A2/tilt and tilt scheme to calculate exposure dependent amplitude attenuation of each spatial frequency')
    ...
    print('Done all configuration steps; starting processing now!')
    t1 = time.time()
    ...
    for i, (batch_idx, batch_ptcls, batch_ctf_params) in enumerate(data_generator):
        print(f'Corrupting particles {(i+1) * args.b}/{particle_dataset.N}')
        ...
        batch_ptcls = torch.fft.fftshift(torch.fft.fft2(torch.fft.ifftshift(batch_ptcls, dim=(-1, -2))), dim=(-1, -2))
        batch_ptcls = batch_ptcls.real - batch_ptcls.imag  # centered hartley transform
        ctf_weights = ctf.compute_ctf(freqs, *torch.split(batch_ctf_params[:, 2:], 1, dim=1), bfactor=args.b_factor).view(-1, D, D)
    ...
    t2 = time.time()
    print(f'Added CTF and noise to {Nimg} images in {t2 - t1}s ({(t2 - t1) / (Nimg)}s per image)')

    # normalize particle stack via subset sampling
    if args.normalize:
        print('Calculating standard deviation of output stack...')
        particles -= np.mean(particles[subset])
        particles /= np.std(particles[subset])
        print(f'Normalized output stack mean and stdev to 0, 1')

    # undo any remaining tilt_series particle reshaping
    particles = particles.reshape(-1, D, D).astype(np.float32)

    # save particles.mrcs
    print(f'Writing image stack to {args.outstack}')
    header = mrc.get_mrc_header(particles, Apix=ctf_params[0,1], is_vol=False)
    ...
    # save ctf.pkl
    if args.out_ctf:
        print(f'Writing ctf parameters to {args.out_ctf}')
